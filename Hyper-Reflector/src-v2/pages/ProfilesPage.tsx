...
import { Award, ArrowRight, Search, Trophy, RefreshCcw, User } from 'lucide-react'
...
const INIT_BOARD: LeaderboardState = { entries: [], cursor: null, loading: false, initialized: false }
...
function UserCard({
  ...
  return (
    ...
    >
      {rank !== undefined && (
        <span className="text-xs w-6 text-right shrink-0 font-mono" style={{ color: 'var(--v2-muted)' }}>
          #{rank}
        </span>
      )}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {user.userTitle?.title && (
            <span
              ...
              style={{
                background: user.userTitle.color || 'var(--v2-hover)',
                color: user.userTitle.textColor || 'var(--v2-text)',
              }}
            >
              {user.userTitle.title}
            </span>
        ...
        {statLine && (
          <p className="text-xs mt-0.5" style={{ color: 'var(--v2-muted)' }}>{statLine}</p>
        )}
        {Array.isArray(user.knownAliases) && user.knownAliases.length > 0 && (
          <p className="text-xs truncate" style={{ color: 'var(--v2-muted)' }}>
            aka {user.knownAliases.slice(0, 3).join(', ')}
          </p>
        )}
      </div>
      {clickable && (
...
export function ProfilesPage({ currentUser, onViewProfile }: ProfilesPageProps) {
  const [searchTerm, setSearchTerm] = useState('')
  const [searchResults, setSearchResults] = useState<SearchUser[]>([])
  const [searchCursor, setSearchCursor] = useState<string | null>(null)
  const [searchLoading, setSearchLoading] = useState(false)
  const [searchError, setSearchError] = useState<string | null>(null)
  const [boards, setBoards] = useState<LeaderboardMap>({ elo: INIT_BOARD, wins: INIT_BOARD })
  ...
  const performSearch = useCallback(async (term: string, reset: boolean, cursor: string | null) => {
  ...
  // Cursor passed as param + initialized set immediately so the useEffect guard
  // fires before the async call resolves — prevents the re-trigger loop.
  const fetchBoard = useCallback(async (kind: 'elo' | 'wins', reset: boolean, cursor: number | null) => {
    if (!auth.currentUser) return
    setBoards(prev => ({ ...prev, [kind]: { ...prev[kind], loading: true, initialized: true } }))
    try {
      ...
      setBoards(prev => ({ ...prev, [kind]: { ...prev[kind], loading: false, initialized: true } }))
    }
  }, [])
  ...
  }, [currentUser?.uid, boards.elo.initialized, boards.wins.initialized, fetchBoard])

  if (!currentUser) {
    return (
      <div className="h-full flex items-center justify-center">
        <p className="text-sm" style={{ color: 'var(--v2-muted)' }}>Sign in to browse profiles.</p>
      </div>
    )
  }

  const renderBoard = (kind: 'elo' | 'wins', icon: React.ReactNode, title: string, desc: string) => {
    const board = boards[kind]
    return (
      <div className="rounded-lg border" style={{ background: 'var(--v2-surface)', borderColor: 'var(--v2-border)' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'var(--v2-border)' }}>
          <div>
            <div className="flex items-center gap-2">
              <span style={{ color: 'var(--v2-accent)' }}>{icon}</span>
              <h2 className="text-sm font-semibold" style={{ color: 'var(--v2-text)' }}>{title}</h2>
            </div>
            <p className="text-xs mt-0.5" style={{ color: 'var(--v2-muted)' }}>{desc}</p>
          </div>
          <button
        ...
        <div className="p-3 space-y-2">
          {board.entries.length === 0 && !board.loading && (
            <p className="text-xs text-center py-4" style={{ color: 'var(--v2-muted)' }}>No players yet.</p>
          )}
          ...
          {board.entries.length > 0 && (
            <button
              ...
              style={{ borderColor: 'var(--v2-border)', color: 'var(--v2-muted)' }}
              onMouseEnter={e => { if (!board.loading && board.cursor) (e.currentTarget as HTMLElement).style.background = 'var(--v2-hover)' }}
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold" style={{ color: 'var(--v2-text)' }}>Player profiles</h1>
        ...
        {/* Search */}
        <div className="rounded-lg border p-4 space-y-3" style={{ background: 'var(--v2-surface)', borderColor: 'var(--v2-border)' }}>
          <div className="relative">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--v2-muted)' }} />
            <input
              type="text"
          ...
          {searchError && <p className="text-xs" style={{ color: '#f87171' }}>{searchError}</p>}
          {trimmed && (
            <div className="flex items-center justify-between">
          ...
          {!searchLoading && trimmed && searchResults.length === 0 && (
            <p className="text-xs py-2" style={{ color: 'var(--v2-muted)' }}>No players found for "{trimmed}".</p>
          )}
          <div className="space-y-2">
            {searchResults.map(user => (
              <UserCard
                ...
                user={user}
                statLine={user.accountElo !== undefined ? `ELO ${user.accountElo}` : undefined}
                onView={onViewProfile}
              />
            ))}
          </div>
        ...
        {/* Leaderboards */}
        <div>
          <h2 className="text-sm font-semibold mb-3" style={{ color: 'var(--v2-text)' }}>Leaderboards</h2>
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            {renderBoard('elo', <Award size={15} />, 'Highest ELO', 'Top rated competitors')}
            {renderBoard('wins', <Trophy size={15} />, 'Most wins', 'Players with the most recorded wins')}
          </div>
        </div>
      </div>
    </div>
