...
import { LogOut, Play, Square, FolderOpen, Volume2, VolumeOff } from 'lucide-react'
    ...
    <div className="space-y-1.5 py-1">
      ...
      {enabled && (
        <div className="flex items-center gap-2 pl-5">
          <p className="text-xs flex-1 font-mono truncate" style={{ color: 'var(--v2-muted)' }}>
            {path || 'Default'}
          </p>
          <button
            onClick={onPick}
...
export function SettingsPage({ user, onLogout }: SettingsPageProps) {
  ...
  const pickSound = async (set: (p: string) => void) => {
    try {
      const res = await open({ multiple: false, directory: false, title: 'Select a sound file', filters: [{ name: 'Audio', extensions: ['mp3', 'wav'] }] })
  ...
  const pickRom = async () => {
    try {
      const res = await open({ multiple: false, directory: true, title: 'Select ROM directory' })
  ...
  const playSound = async (path: string) => {
    if (!path) return
    try { await invoke('play_sound', { path }) } catch { /* no-op */ }
  }

  const stopSound = async () => {
    try { await invoke('stop_sound') } catch { /* no-op */ }
  }

  const handleReset = async () => {
    if (!resetConfirm) { setResetConfirm(true); return }
    try {
      const store = useSettingsStore as typeof useSettingsStore & { persist?: { clearStorage?: () => void } }
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-lg mx-auto p-6 space-y-4">
        ...
        <Section title="Account">
          <Row label="Username">
            <span className="text-sm font-medium" style={{ color: 'var(--v2-accent)' }}>{user.userName}</span>
          </Row>
          {user.userEmail && (
            <Row label="Email">
        ...
        <Section title="Appearance">
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {THEMES.map(t => (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium" style={{ color: 'var(--v2-text)' }}>{t.name}</span>
                    <div className="flex gap-1 mt-1.5">
                      {(['--v2-bg', '--v2-accent', '--v2-name-self'] as const).map(k => (
                        <span key={k} className="w-3 h-3 rounded-full border border-white/10" style={{ background: t.vars[k] }} />
                      ))}
                    </div>
                  </button>
                ))}
            ...
            <div className="pt-3 border-t space-y-3" style={{ borderColor: 'var(--v2-border)' }}>
              ...
              <ColorSwatch label="Chat text"     current={currentChatColor}  swatches={CHAT_MSG_SWATCHES}  onChange={c => setOverride('--v2-chat-msg', c)} />
              <ColorSwatch label="My username"   current={currentSelfColor}  swatches={NAME_SELF_SWATCHES} onChange={c => setOverride('--v2-name-self', c)} />
              <ColorSwatch label="Others' names" current={currentOtherColor} swatches={NAME_OTHER_SWATCHES} onChange={c => setOverride('--v2-name-other', c)} />
              {/* Chat preview */}
              <div className="text-xs rounded px-3 py-2 border" style={{ borderColor: 'var(--v2-border)', background: 'var(--v2-hover)' }}>
        ...
        <Section title="Notifications">
          <SoundRow
            ...
            onStop={stopSound}
          />
          <div className="border-t" style={{ borderColor: 'var(--v2-border)' }} />
          <SoundRow
        ...
        <Section title="Files">
          <div className="space-y-1 py-1">
            <p className="text-sm" style={{ color: 'var(--v2-text)' }}>ROM directory</p>
        ...
        <Section title="App">
          ...
          <Row label="Version">
            <div className="flex rounded overflow-hidden border text-xs font-medium" style={{ borderColor: 'var(--v2-border)' }}>
              <button
          ...
          <div className="space-y-2 py-1">
            ...
            <button
              ...
              style={{
                ...
                background: resetConfirm ? 'rgba(239,68,68,0.1)' : 'var(--v2-hover)',
              }}
            >
